"""Visualise output of condensation when using bifiltration callbacks."""


# TODO:
# - look at .npz file and understand format for bifiltration information
# - adapt script below to read in correct data
# - find and implement a visualization method from RIVET paper
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--interval", default=200, type=int, help="Update interval"
    )
    ...
    assert ( #Change naming to check for bifiltration pairs
        "bifiltration_persistence_pairs" in parsed_keys
    ), 'Require "bifiltration_persistence_pairs" key'
    ...
    total_persistence = [np.sum(np.diff(pd[pd[:, 1] <= t])) for t in range(T)]
