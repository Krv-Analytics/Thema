...
class JGraph:
    ...
    def get_MST(self, k=0, components=None):
        ...
        # Handle component specific 
            ...
            assert len(k) == len(components), 'Length of k must be equal to length of components'
            ...
            for i in range(len(self._components)): 
                ...
                if i in components: 
                    ...
                    cc_sorted_edges = sorted(cc_mst.edges(data=True), key=lambda x: x[2]["weight"])
                    
                    # Check number of edges is greater than k value 
                    assert k[j] < len(cc_sorted_edges), f"k value for component {i} was greater than number of edges in component. "
