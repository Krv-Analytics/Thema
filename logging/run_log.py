...
#####################################################################################
#
#  Functions to interact with parmater yaml file
#
#####################################################################################
...
def get_default_jmap_log():
    ...
    return os.getenv("root") + params["jmap_runlog"]
...
def get_default_projection_log():
    ...
    with open(YAML_PATH, "r") as f:
        params = OmegaConf.load(YAML_PATH)
    return os.getenv("root") + params["projection_runlog"]
...
def get_default_runlog_dir():
    ...
    with open(YAML_PATH, "r") as f:
        params = OmegaConf.load(YAML_PATH)
    return os.getenv("root") + params["runlog_dir"]
...
class Run_Log:
    ...
    def __init__(
        self,
        jmap_runlog: str = get_default_jmap_log(),
        projection_runlog: str = get_default_projection_log(),
        runlog_dir: str = get_default_runlog_dir(),
    ):
        ...
        if not isfile(self._projection_runlog):
            ...
            if os.path.isdir(self._runlog_dir):
                with open(self._projection_runlog, "w") as f1:
                    OmegaConf.save(projection_runlog_contents, f1)
            else:
                os.makedirs(self._runlog_dir, exist_ok=True)
                with open(self._projection_runlog, "w") as runlog:
                    OmegaConf.save(projection_runlog_contents, runlog)
